...
public class SecurityConfig {
	...
	public UserDetailsService configAuthentication() throws Exception {
		...
		UserDetails user1 = User.withUsername("student").password(encoder().encode("student")).roles("STUDENT").build();

		UserDetails user2 = User.withUsername("admin").password(encoder().encode("admin")).roles("ADMIN").build();

		UserDetails user3 = User.withUsername("employee").password(encoder().encode("employee")).roles("EMPLOYEE")
				.build();
